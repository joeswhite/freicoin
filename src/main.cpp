...
{
    mpq nSubsidy = 0;
    if ( nHeight < EQ_HEIGHT ) // EQ_HEIGHT can be any block number we want to cut funds off from the foundation and fork. this would have the unintended consequence of changing the total number of coins on the chain
//      if ( nHeight < 62047 ) // fork off in about 1 months time at normal block count. approx may 9th 2014 @ 0900 EDT (GMT -4:00_
        nSubsidy = TITHE_AMOUNT + (EQ_HEIGHT-nHeight) * INITIAL_SUBSIDY / EQ_HEIGHT;
    return nSubsidy;
}

CBudget static GetInitialDistributionBudget(int nHeight) // if we wanted to, we could rip out all of these addresses and put in eater addresses to keep the total coinbase @ 100 million.
{
    static CBitcoinAddress vAddresses[320] = {
        ...
        CBitcoinAddress("1E6WgpC4bmYJagvsTzhRxZ1Z8sRSsQjmJX"),//last foundation address to get coins. rest go to a coin eater/black hole address and will demurrage back in to the system over 20 years time
        CBitcoinAddress("1111111111111111111114oLvT2"),
        CBitcoinAddress("1111111111111111111114oLvT2"),
        CBitcoinAddress("1111111111111111111114oLvT2"),
        CBitcoinAddress("1111111111111111111114oLvT2"),
        CBitcoinAddress("1111111111111111111114oLvT2"),
